...
vector<SpriteAnimation*> SpriteAnimation::SpriteAnimationList;
...
{
	if (Flipped)
		return *Frames[CurrentFrame + TotalFrames];
	else
...
// Frames dont always have the same height. 
// This offset is the amount of pixels needed to put the frame aligned to the bottom of the sprite.
int SpriteAnimation::FrameOffsetY(int spriteHeight)
{
	int offset = 0;
	int frameHeight = Frames[CurrentFrame]->h;
	
	if (frameHeight < spriteHeight)
	{
		offset = spriteHeight - frameHeight;
	}
...
{
	if (LastAnimationTick == 0 || SDL_GetTicks() - LastAnimationTick >= (unsigned int)FrameRate)
	{
		NextFrame();
...
void SpriteAnimation::InitAnimations()
{
	...
	//
	// F:<frame_rate>;L:<loop>;				-- General properties
	...
	for (int i=0; i<(int)definition.size(); i++)
	{
		...
		if (line.length() > 0 && line.substr(0, 2) != "//")
		{
			...
			else if (endSequence != -1)
			{
			    ...
			    SpriteAnimation::SpriteAnimationList.push_back(spriteAnimation);
			}
			else
			{
